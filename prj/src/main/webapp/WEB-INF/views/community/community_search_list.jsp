...
<body>
	...
	<div id="main_container">
		...
		<div class="header2">커뮤니티 게시판 내 검색 결과입니다.</div>
							...
							<tr>
								...
								<td class="community_title">
									<a id="community_title" href="<%=request.getContextPath() %>/community_content.do?no=${dto.community_num }&page=${paging.page }">
										...
										<c:if test="${dto.community_hit > 100 }">
								...
								<c:if test="${!empty dto.community_writer }">
									<td><a href="<%=request.getContextPath() %>/member_profile_view.do?user_nick_name=${dto.community_writer}">${dto.community_writer }</a></td>
								</c:if>
				...
				<div class="bestListWrap">
					...
					<div class="bestListInner">
						<c:if test="${!empty bestList }">
							<c:set var="i" value="1" />
							<c:forEach items="${bestList }" var="bdto" begin="0" end="14">
								<p class="rank">
									<em>${i }</em>
