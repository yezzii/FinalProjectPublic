...
<body>
	...
	<div class="report-pwd-con" align="center">
		<form method="post"
			action="<%=request.getContextPath()%>/manager_user_stop_ok.do">
			<input type="hidden" name="user_id" value="${mDto.id}"> <input
				type="hidden" name="user_nick_name" value="${mDto.nick_name}">
			<input type="hidden" name="db_pwd" value="${dto.manager_password}">
			<h3>
				접수된 ${mDto.id}회원의 신고 처리를 진행합니다.<br>계정을 정지시키려면 관리자 비밀번호를 입력해
				주세요.
			</h3>
