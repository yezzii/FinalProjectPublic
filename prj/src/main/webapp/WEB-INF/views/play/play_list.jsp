...
<body>
	...
	<form method="post" action="<%=request.getContextPath()%>/SearchResultOk.do" id="location-form" class="search-form">
				...
				<div class="price-input">
	 			...
	 			<input class="play_button" type="submit" value="검색"> 
					<!-- <button class="play_button" onclick="submit">검색</button> -->
 					<span class="list_container" id="lens-icon"> 
					<i class="fa fa-search" id="search_list"></i>
					</span> 
				</div>
			</div>
		</div>
			...
			<div class="wrapper">
				...
				<div class="price-input1">
					...
					<div class="field1 input-max-wrap1">
						<input type="number" class="input-max1" name="distance_input" value="500"> <span>KM</span>
					</div>
					<input class="play_button" type="submit" value="검색">
				</div>
			</div>
	...
	<div class="search-container" id="search_container">
		<form method="post"
			action="<%=request.getContextPath()%>/list_search.do">
		...
		<div id="playlist-container" class="list_wrap">
			<c:forEach items="${list }" var="dto" varStatus="status">
				<div class="play_list" data-address="${dto.play_address }">
					...
					<span class="play_star">⭐</span><span class="play_starfont">&nbsp;<fmt:formatNumber value="${dto.play_rating}" pattern="0.0" /></span>	
					&nbsp;<img class="play_view"
						src="./resources/assets/play_pic/play_view.png"> <span
						class="play_viewfont">${dto.play_view }</span> 
	...
	<%@include file="../include/footer_play_list.jsp"%>
...
/*검색 */
document.getElementById("lens-icon").addEventListener("click", function () {
	  var searchForm = document.getElementById("search_container");
