let idPattern = /^[A-Za-z0-9_\-]{5,20}$/; // 영문 대,소문자, 숫자, 특수문자(_,-)만 입력 가능하고 5에서 20자리를 입력했는지 체크한다
...
$(function() {
	...
	$('#id').focusout(function() {
		$.ajax({
			...
			type: "post",
			url: "member_checkInsertId.do",
			data: {"inputId": $("#id").val()},
			datatype: "json",
			success: function(data) {
				if(!idPattern.test($("#id").val()) || $("#id").val() === "") {
					idChecked = false;
					let warningTxt = '<font color = "red">5~20자리의 영문 대, 소문자, 숫자, 특수문자(-, _)만 사용 가능합니다.</span>';
					$("#checkId").text(""); // span 태그 영역 초기화
					$("#checkId").show();
					$("#checkId").append(warningTxt);
				} else {
					if (data === '{"resultChk":"중복"}') {
						// 중복인 경우
						idChecked = false;
						let warningTxt = '<font color = "red">중복 아이디입니다.</span>';
						$("#checkId").text(""); // span 태그 영역 초기화
						$("#checkId").show();
						$("#checkId").append(warningTxt);
					} else {        
						idChecked = true;
						let warningTxt = '<font color = "green">사용 가능한 아이디입니다.</span>';
						$("#checkId").text(""); // span 태그 영역 초기화
						$("#checkId").show();
						$("#checkId").append(warningTxt);
					}
		...
		$.ajax({
			...
			success: function(data) {
				if(!pwdPattern.test($("#password").val()) || $("#password").val() === "") {
					pwdChecked = false;
					let warningTxt = '<font color = "red">8~16자리의 영문 대, 소문자, 숫자, 특수문자를 하나 이상 포함하여<br> 사용 가능합니다.</span>';
